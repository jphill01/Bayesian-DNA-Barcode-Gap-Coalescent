...
A number of summary quantities were reported, including posterior means, posterior SDs, and posterior quantiles from which 95\% CrIs could be computed to make probabilistic inferences concerning true population parameters.

To validate the overall correctness of the proposed statistical model given by \\ Equation (6), in addition to generating MLEs using Equations (1)-(4) and ECDFs via {\tt ggplot2}'s {\tt stat\_ecdf()} function \citep{wickham2016ggplot2}, as a means of comparison, posterior predictive checks were also employed to generate binomial random variates in the form of counts from the posterior predictive distribution; that is \\ $\gamma = \{Np_x, Mq_x, Np^{'}_x, Cq^{'}_x\}$ to verify that the model adequately captures relevant features of the observed data.
...
\textbf{Parameter} & \textbf{MLE/ECDF (95\% CI)} & \textbf{Bayes Est. (95\% CrI)} \\  \hline
$p_\mathrm{lwr}$ & & \\
$p_\mathrm{upr}$ & & \\
$p^{'}_\mathrm{lwr}$ & & \\
$p^{'}_\mathrm{upr}$ & & \\
...
\caption{Nonparametric frequentist and Bayesian estimates of genetic distance distribution overlap/separation for the DNA barcode gap coalescent model parameters, including 95\% confidence and credible intervals.}
