...
\noindent \noindent which has support in [0, 2]. Similarly, the degree of overlap is 

\begin{align}
O &= |p_x - q_x| = |1 - \hat{F}_{d_{ij}}(a) - \hat{F}_{d_{XY}}(b)| = |\hat{F}_{d_{ij}}(b) - \hat{F}_{d_{ij}}(a)  - \hat{F}_{d_{XY}}(b)|\\
O' &= |p'_x - q'_x| =  |1 - \hat{F}_{d_{ij}}(a') - \hat{F}_{d_{XY}}(b)| = |\hat{F}_{d_{ij}}(b) - \hat{F}_{d_{ij}}(a')  - \hat{F}_{d_{XY}}(b)|.
\end{align}

\noindent which has support in the unit interval. Equations (5)-(12) clearly demonstrate the asymmetric directionality of the metrics.
