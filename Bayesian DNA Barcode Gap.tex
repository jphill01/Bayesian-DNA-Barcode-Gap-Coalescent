...
To validate the overall correctness of the proposed statistical model given by \\ Equation (6), in addition to generating MLEs using Equations (1)-(4) as a means of comparison, posterior predictive checks were also employed to generate binomial random variates in the form of counts from the posterior predictive distribution; that is $\gamma = \{Np_x, Mq_x, Np^{'}_x, Cq^{'}_x\}$ to verify that the model adequately captures relevant features of the observed data.
