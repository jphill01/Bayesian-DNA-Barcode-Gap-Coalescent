...
To validate the overall correctness of the proposed statistical model given by Equation (6), as a means of comparison, posterior predictive checks were also employed to generate binomial random variates in the form of counts from the posterior predictive distribution; that is $\gamma = \{Np_x, Mq_x, Np^{'}_x, Cq^{'}_x\}$ to verify that the model adequately captures relevant features of the observed data.
...
\caption{Nonparametric frequentist and Bayesian estimates of genetic distance distribution overlap/separation for the DNA barcode gap coalescent model parameters, including 95\% confidence and credible intervals. CrIs are based on 4000 posterior draws.}
