...
``Analysis.R` cntains one function, `run_DNA_barcode_gap_analysis_by_marker()`, which prompts users interactively to select the folder containing marker subfolders (*e.g.*, COI-3P, COI-5P, CYTB) and the `DN_barcode_gap.stan` program located on their desktop. 
